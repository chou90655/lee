  ...
  <div class="display" id="xgc">
    ...
    <cube-scroll class="main_play" :options="options" ref=mainPlay :class="[change && 'change']">
      ...
      <ul v-if='rD.ball || (rD.balls && rD.balls[rodioIndex])' :class="['ball', rD.odds && 'hasOdd']">
        <li v-for="(it, i) in rD.ball || (rD.balls && rD.balls[rodioIndex])" @click="handleChose(it)" :key=i :class="[it.choose && 'chosen', it.lock && 'lock', getColor(it.name)]">
          <p>{{it.name}}</p>
          <i>{{it.odds}}</i>
        </li>
      </ul>
      ...
      <ul v-for="(item, i) in rD.sort || (rD.sorts && rD.sorts[rodioIndex]) || []" :key="i" :class="{square: item.square, ball: item.ball, _first: i===0}">
        ...
        <li v-for="(it, i) in item.square || item.ball" @click="handleChose(it, item.square)" :key=i :class="{t_b: change && it.choose}">
          <p :class="it.choose && !change ? 't_b': 't_bd'">{{it.name}}</p>
          <i :class="[it.choose ? 't_bc' : '', !change && 't_bd']">{{it.odds}}</i>
        </li>
      </ul>
      <ul v-for="(item, i) in rD.group || (rD.groups && rD.groups[rodioIndex]) || []" :key="i" :class="['group', item.lock && 'lock', item.choose && 'chosen', item.numbers.length > 5 && 'many']" @click="handleChose(item)" >
        <icon href='select'/>
